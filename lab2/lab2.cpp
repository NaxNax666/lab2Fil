...
int main(int argc, char** argv)
{
    ...
    {
        for (i = 0; i < N1; i++) {
            for (j = 0; j < N2; j++) A[i][j] = i + j; // Manunannaayna maccnea A
            B[i] = i;                                             // Wanqnannzayna maccnea B
        }
        ...
        for (cicle = 0; cicle < 10000; cicle++) // «wxaxpatHoctn»
        {
            ...
            for (i = 1; i < size; i++) // C6op peaynbtatos 8 maccns C
        ...
        time = time2 - time1;
        for (i = 0; i < N1; i++) printf("%f", C[i]); // BuBon pesyrbtaToe
    ...
    {
        for (i = 0; i < N1; i++)
            for (j = 0; j < N2; j++) A[i][j] = i + j;  // Wnnynannaayna maccnea A
            // 0 scex npoyecax
